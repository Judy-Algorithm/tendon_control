...
- 通路名称下的「模型起止」来自同一 SHM 模型的首、末 GeometryPath 路径点，显示端点所属骨体。`endpoint-data.js` 保留模型摘要、骨体 ID 和路径点名称，37 条通路均已与源文件核对。它不是人体解剖起止点，也不是左侧公开 MyoHand 几何的附着说明；等效通路保留模型实际附着，不按教科书改写。
