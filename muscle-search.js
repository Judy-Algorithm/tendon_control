export class MuscleSearch {
  constructor(atlas,tendons,onChange){
    this.atlas=atlas;this.tendons=new Map(tendons.map(t=>[t.modelName??t.id,t]));this.onChange=onChange;
  ...
  refresh(){
    ...
    if(!search){this.status.textContent='输入 OpenSim 模型中的肌肉名称，在手模上查看对应通路';return;}
    const ids=this.atlas.scope();
    this.status.textContent=ids.length===0?'未找到该缩写，请检查拼写（如 FDS3、FDP5）':
      ids.length===1?'已显示 1 条 OpenSim 肌肉通路':`已显示 ${ids.length} 条 OpenSim 肌肉通路，点击一项可单独查看`;
    for(const id of ids){
      ...
      button.setAttribute('aria-label',`单独显示 OpenSim 肌肉 ${id}，中文参考 ${meta.name}`);
      ...
      code.textContent=meta.modelName??id;name.textContent=`中文参考：${meta.name}`;
