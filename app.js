...
const visualMuscles=[...new Map([...ATLAS.tendons,...OPENSIM_MUSCLES].map(t=>[t.id,t])).values()];
const tendonMeta=new Map(visualMuscles.map(t=>[t.id,t]));
const specialColors={FDS3:'#ef6975',FDP3:'#eeb85b',EDC3:'#6aa2fa',RI3:'#57cca0',LU_RB3:'#b68af0',UI_UB3:'#61d7df',APB:'#f08eb8',FPB:'#f0a35b'};
const colorFor=id=>{
  const meta=tendonMeta.get(id),seed=meta.modelIndex??[...id].reduce((sum,char)=>sum+char.codePointAt(0),0);
  return specialColors[id]||'hsl('+(Math.round(seed*137.508)%360)+',65%,66%)';
};
...
class TendonViewer {
  constructor(){
    ...
    for(const meta of visualMuscles){
      ...
      const segments=[];
      const fitted=meta.route??FITTED_ROUTES[meta.sourceId??meta.id];
