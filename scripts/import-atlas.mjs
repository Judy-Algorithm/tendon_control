...
for(const digit of Object.keys(parts)){
  ...
  names['RI'+digit]=`${parts[digit]}桡侧骨间肌等效通路`;
  names['LU_RB'+digit]=`${parts[digit]}蚓状肌—桡侧腱膜通路`;
  names['UI_UB'+digit]=`${parts[digit]}尺侧骨间肌—尺侧腱膜通路`;
}
...
await fs.writeFile('docs/data-provenance.json',JSON.stringify({geometry:{repository:'https://github.com/Judy-Algorithm/EMG2Tendon',commit:'0513b54',source:'public MyoHand'},
  ...
  rendering:'The public MyoHand surface and tendon paths are a visual reference. The 16-joint/23-DoF ranges and associations are from the SHM OpenSim control table. No pose simulation or native MyoHand moment-arm claims are made.'},null,2)+'\n');
